...
# Startup script to initialize and boot a bera-reth instance.
#
# This script assumes the following files:
#  - `bera-reth` binary is located in the filesystem root
...
#
# These flags are NOT supported by bera-reth
#
...
bera_reth=/usr/local/bin/bera-reth
...
DATADIR="/bera-reth-hive-datadir"
mkdir $DATADIR
...
$bera_reth init $FLAGS --chain /genesis.json
...
if [ -f /chain.rlp ]; then
    RUST_LOG=info $bera_reth import $FLAGS /chain.rlp
else
...
if [[ ! -d "/blocks" || ${#BLOCKS[@]} -eq 0 ]]; then
    ...
    $bera_reth import $FLAGS "${BLOCKS[0]}"
else
    ...
    $bera_reth import $FLAGS "combined.rlp"

    # Import the last block separately
    $bera_reth import $FLAGS "${BLOCKS[-1]}"
fi
...
echo "Running bera-reth with flags: $FLAGS"
RUST_LOG=info $bera_reth node $FLAGS