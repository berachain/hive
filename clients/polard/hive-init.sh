...
# HIVE assumes that the chain ID is 1337
jq '.app_state.evm.params.eth_genesis |= (fromjson | .config.chainId = 1337 | tostring)'  "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
...
# Test Accounts
polard genesis add-genesis-account polar1vk9a7s6asyxfzs2wczg50k4xmd3yqcmex342hu 69000000000000000000000000abera --keyring-backend $KEYRING --home "$HOMEDIR"
polard genesis add-genesis-account polar1eaylmga7x57xndq7m93n8nfyxqk6g4t05w2f7t 69000000000000000000000000abera --keyring-backend $KEYRING --home "$HOMEDIR"
